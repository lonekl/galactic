...
impl Opt {
	...
	pub fn new() -> Result<Self, Vec<String>> {
		...
		let mut arg_state = ArgState::Task;
		...
		let mut cargo_bin: PathBuf = home_dir.clone();
				...
				match &mut arg_state {
					...
					ArgState::Task => {
						...
						match arg_char {
							...
							'[' => arg_state = ArgState::Config { flag_part: false },
							' ' => {},
							_ => errors.push(format!("There is no such task as \'{arg_char}\'.")),
						}

						if error_count_before == errors.len() && arg_state == ArgState::Task {
